...
best=0
model = BiLSTM_CRF(len(word2id), tag_to_ix, EMBEDDING_DIM, HIDDEN_DIM,embedding_matrix=embeddings_matrix)
...
try:
    for epoch in range(100):
        for i in range(len(X_train)):
            ...
            loss = model.neg_log_likelihood(X_train[i][0],X_train[i][1],numer_Y)
        ...
        if f1>best:
            best=f1
            torch.save(model.state_dict(), 'bestmodel_lstmcrf')

except KeyboardInterrupt:
    print ("----------------- INTERRUPTED -----------------")
    model.load_state_dict(torch.load('bestmodel_lstmcrf'))
...
print(files_len_list)
start=0
for i in range(len(files_len_list)):
    final_data.append((torch.tensor(X[start:start+files_len_list[i]]),torch.tensor(Y[start:start+files_len_list[i]])))
    start+=files_len_list[i]

batchList=batchDataset(final_data)
"""
#for i in range(len(final_X)):
#    print(final_X[i].shape,final_Y[i].shape)
